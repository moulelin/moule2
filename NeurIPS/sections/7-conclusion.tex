...
\begin{itemize}
    \item \textbf{Scalability}: Current experiments use 4 GPUs; scaling to multi-node setups requires careful optimization of communication backends (NCCL vs. Gloo)
    \item \textbf{Generalization}: We focus on mathematical reasoning (GSM8K); extending to other reasoning tasks (code, commonsense) is important
    \item \textbf{Teacher quality}: Our approach assumes access to a capable teacher; exploring self-improvement scenarios is promising
\end{itemize}
