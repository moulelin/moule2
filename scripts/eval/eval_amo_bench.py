"""
Evaluate on AMO-Bench (meituan-longcat/AMO-Bench).

Usage:
  python eval_amo_bench.py --model Qwen/Qwen3-1.7B --mode greedy
  python eval_amo_bench.py --model Qwen/Qwen3-8B --mode majority --n_samples 16
"""
...
HF_REPO = "meituan-longcat/AMO-Bench"
SPLIT = "test"
PROBLEM_KEY = "prompt"
ANSWER_KEY = "answer"
DATASET_NAME = "amo_bench"
...
def evaluate(args):
    ...
    for i, output in enumerate(outputs):
        ...
        if args.mode == "greedy":
            ...
            print(f"\n--- Problem #{i} ---")
            print(f"  GT (raw):        {gt!r}")
            print(f"  GT (stripped):   {gt_stripped!r}")
            print(f"  Pred (extracted+stripped): {predicted!r}")
            ...
            print(f"  GT (raw):        {gt!r}")
            print(f"  GT (stripped):   {gt_stripped!r}")
            for j, s in enumerate(per_sample):
                print(f"  Sample {j}: extracted+stripped={s['extracted']!r}  correct={s['correct']}")
            # ---- END DEBUG ----
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate on AMO-Bench")
