...
class DeepspeedStrategy(ABC):
    ...
    def setup_distributed(self, timeout=timedelta(minutes=60)) -> None:
        ...
        # In Ray environment, use Ray's GPU assignment (via CUDA_VISIBLE_DEVICES)
        # Don't use LOCAL_RANK which may conflict with Ray's allocation
        import ray
        if ray.is_initialized():
            # Ray handles GPU assignment via CUDA_VISIBLE_DEVICES
            # Use the first visible device (index 0 in the actor's view)
            if torch.cuda.is_available() and torch.cuda.device_count() > 0:
                torch.cuda.set_device(0)  # First GPU in CUDA_VISIBLE_DEVICES
        else:
            # Non-Ray environment: use LOCAL_RANK
