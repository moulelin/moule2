...
def blending_datasets(
    ...
    for i, dataset in enumerate(datasets):
        ...
        # remote/local folder or common file
            ...
            # Handle dataset configs (e.g., "openai/gsm8k:main")
            dataset_name = dataset
            dataset_config = None
            if ":" in dataset:
                dataset_name, dataset_config = dataset.rsplit(":", 1)
            # Special handling for datasets that require configs
            elif dataset == "openai/gsm8k":
                dataset_config = "main"

            if dataset_config:
                data = load_dataset(dataset_name, dataset_config, data_dir=data_dir)
                strategy.print(f"loaded {dataset_name} (config: {dataset_config})")
            else:
